...
def plot_katsner_curve(df, AwalBulan, bulan_hijriah, tahun_hijriah, alat='TELESCOPE'):
    ...
    sns.lineplot(x='T AFTER SUNSET', y=f'{alat} (K2)', data=df, color='blue', linestyle='-', marker = 'o', label='Atmosfer dengan k = 0,2 (Bersih)', ax=ax)
    sns.lineplot(x='T AFTER SUNSET', y=f'{alat} (K4)', data=df, color='orange', linestyle='-', marker = 'o', label='Atmosfer dengan k = 0,4 (Moderat)', ax=ax)
    sns.lineplot(x='T AFTER SUNSET', y=f'{alat} (K6)', data=df, color='red', linestyle='-',  marker = 'o', label='Atmosfer dengan k = 0,6 (Kotor)', ax=ax)

    ax.set_xlabel('Menit (Setelah Matahari Terbenam)', fontsize=12)
    ax.set_ylabel('Visibilitas', fontsize=12)

    bulan_hijriah_str = hijriah().bulan_hijriah(bulan_hijriah)

    plt.title(f'KURVA VISIBILITAS MODEL KATSNER \n PENENTUAN AWAL BULAN {bulan_hijriah_str.upper()} {tahun_hijriah} H \n {jd_caldat_result.upper()} \n', fontsize=14, pad = 20)
    ...
    plt.figtext(0.46, -0.08, f"First Visibility :\n     Kondisi Bersih    : {first[0]}\n     Kondisi Moderat : {first[1]}\n     Kondisi Kotor      : {first[2]}", fontsize=10)
    plt.figtext(0.68, -0.08, f"Best Time :\n     Kondisi Bersih    : {best[0]}\n     Kondisi Moderat : {best[1]}\n     Kondisi Kotor      : {best[2]}", fontsize=10)
...
def plot_hilal_map(data_peta, bulan_hijriah_str, tahun_hijriah, data_type="altitude"):
    ...
    if data_type.lower() == "altitude":
        ...
        title_type = "KETINGGIAN HILAL (TOPOSENTRIS)"
        ...
        title_type = "ELONGASI (GEOSENTRIS)"
    else:
    ...
    contour = ax.contour(longitudes, latitudes, plot_data, levels=5, cmap=cmap, norm=norm, transform=ccrs.PlateCarree())
...
tahun_input = int(st.sidebar.number_input("Tahun Hijriah", min_value=1400, value=1446, step=1))
...
TZ_input = st.sidebar.selectbox("Timezone", ["Asia/Jakarta", "Asia/Makassar", "Asia/Jayapura"], index=0)
TT_input = int(st.sidebar.number_input("Elevasi (m)", value=109,step=1))
TH_input = int(st.sidebar.number_input("Tambah Hari", value=0))
kriteria_input = st.sidebar.selectbox("Kriteria Visibilitas", ["NEO MABIMS", "IRNU", "MUHAMADIYYAH", "MABIMS LAMA"], index=0)
alat_katsner_input = st.sidebar.selectbox("Modus Katsner Plot", ["NAKED EYE", "THEODOLIT", "TELESCOPE"], index=0)
...
if st.sidebar.button("Hitung"):
    ...
    st.code(f"""
        --------------------------------------------------------------------------------
                                     DATA FALAKIYAH HILAL
                               AWAL BULAN {(hijriah().bulan_hijriah(bulan_input)).upper()} {tahun_input} H
                 JET PROPULSION LABORATORY (JPL) NASA, BY FIKA AFHAMUL FUSCHA
        --------------------------------------------------------------------------------

        Perhitungan dilakukan untuk waktu matahari terbenam pada {data['sunset'].strftime('%H:%M:%S')}, tanggal {data['sunset'].day} {data['n_bln']} {data['sunset'].year} M
        Semua data disajikan dalam waktu lokal pengamat
